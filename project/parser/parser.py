...
class FTModel(model):
    def __init__(self, url, name, spldata, objnum=1):
        self.name = name
        self.url = url
        self.ft = load_ft_url(url)
        self.ft.loadCost(spldata)
        dec = [Has(l.id, 0, 1) for l in self.ft.leaves]
        obj1 = Has(name='fea', lo=0, hi=self.ft.featureNum - len(self.ft.groups), goal=lt)
        obj2 = Has(name='conVio', lo=0, hi=len(self.ft.con), goal=lt)
        obj3 = Has(name='cost', lo=0, hi=sum(self.ft.cost), goal=lt)
        obj4 = Has(name='time', lo=0, hi=sum(self.ft.cost), goal=lt)  # TODO CHANGE THE COST TO TIME FOR OBJ4
        if objnum == 1: obj = [obj1]
        if objnum == 2: obj = [obj1, obj2]
        if objnum == 3: obj = [obj1, obj2, obj3]
        if objnum == 4: obj = [obj1, obj2, obj3, obj4]
    ...
    def eval(self, c, doNorm=True, returnFulfill=False):
        ...
        obj1 = self.obj[0].hi - (sum(fulfill) - gsum)
    ...
    def __repr__(self):
        info = ''
        info += '---Information for ' + self.name + '------\n'
        info += 'Leaves #:' + str(len(self.ft.leaves)) + '\n'
        info += 'Total Features#:' + str(self.ft.featureNum - len(self.ft.groups)) + '\n'
        info += 'Constraints#:' + str(len(self.ft.con)) + '\n'
        info += '-' * 30
        return info
